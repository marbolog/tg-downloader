...
### Container behaviour
- `restart: always` — containers restart automatically on crash or server reboot. **It does not react to a Docker healthcheck reporting `unhealthy`** — that only ever restarts a container whose process actually exits. A hung-but-alive process (see below) needs something else watching health status to get restarted automatically: the `autoheal` service (`willfarrell/autoheal`, third compose service) does this — it holds the Docker socket and runs `docker restart` on any container labeled `autoheal=true` (currently just `tg-downloader`) whenever its healthcheck reports unhealthy.
...
- **Healthcheck**: `data/.heartbeat` is touched by `listener._touch_heartbeat`: once after *each individual* item during `_flush_pending`/`_heal_missing`/`_backfill_missed`/`_deep_reconcile` (via the shared `_gather_with_heartbeat` helper), and again on every hourly heartbeat. `docker-compose.yml`'s healthcheck fails if that file is older than 90 minutes. Incident 2026-08-21..26: after a `ConnectionError` at startup, a single stalled network call inside `_flush_pending` hung forever with no timeout — and since `_flush_pending`/`_heal_missing`/`_backfill_missed` are all awaited *before* `_heartbeat_loop` (or any other background task) is created, nothing downstream ever ran. The container reported "Up" the whole time; every channel silently stopped receiving new files for 5 days. Fixed by `download.timeout_seconds` (below) bounding every network call `download_item` makes, so `_flush_pending` etc. can no longer hang indefinitely; the healthcheck is defense-in-depth for hang classes the timeout doesn't cover (e.g. a synchronous PyMuPDF call in `analyze_file` blocking the event loop).
- **Follow-on incident, same day**: the first version of the healthcheck only touched `data/.heartbeat` once *all* of flush/heal/backfill finished. `autoheal` (below) restarted the listener mid-recovery because a large backlog flush (64 items, `concurrent_downloads: 1`, some 80+ MB files) legitimately took longer than `start_period` (30m) — genuine progress looked identical to a hang from the healthcheck's point of view. Fixed by touching the heartbeat after every individual item instead of only at the end, so forward progress itself is the health signal regardless of how long a backlog recovery takes.
...
`download.timeout_seconds` (default 600) bounds every network call `download_item` makes — entity resolution, message lookup, and the media download itself — via `asyncio.wait_for`. Without this, a single stalled connection hangs that call forever; see the Container behaviour incident note above for what that cost when it happened during startup.
